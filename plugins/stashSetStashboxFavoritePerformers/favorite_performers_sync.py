...
def update_stashbox_performer_favorite(endpoint, boxapi_key, stash_id, favorite):
    query = """
...
def get_favorite_performers_from_stashbox(endpoint, boxapi_key):
    query = """
...
def set_stashbox_favorite_performers(db: StashDatabase, endpoint, boxapi_key):
...
WHERE a.favorite = 1""")])
    ...
    for stash_id in favorites_to_add:
        update_stashbox_performer_favorite(endpoint, boxapi_key, stash_id, True)
    log.info('Add done.')
